...
interface HistoryLog {
    action: string;
    layer?: Selected;
    oldLatLngs?: L.LatLngExpression[] | L.LatLngExpression[][] | L.LatLngExpression[][][];
    newLatLngs?: L.LatLngExpression[] | L.LatLngExpression[][] | L.LatLngExpression[][][];
    oldLatLng?: L.LatLngExpression;
    newLatLng?: L.LatLngExpression;
    oldMapInfo?: PLAComponent;
    newMapInfo?: PLAComponent;
}

let editHistory: HistoryLog[] = [];
let undoStack: HistoryLog[] = [];

function undo() {
    let prevLog = editHistory.pop();
    if (prevLog === undefined) return;
    ...
    if (prevLog.action == "create") {
        ...
        if (prevLog.oldLatLngs !== undefined) prevLog.layer.setLatLngs(prevLog.oldLatLngs);
        // @ts-ignore
        else prevLog.layer.setLatLng(prevLog.oldLatLng);;
...
function redo() {
    ...
    if (prevLog.action == "create") {
        ...
        if (prevLog.newLatLngs !== undefined) prevLog.layer.setLatLngs(prevLog.newLatLngs);
        // @ts-ignore
        else prevLog.layer.setLatLng(prevLog.newLatLng);;
        prevLog.layer.fire("click");
    } else if (prevLog.action == "modifyMapInfo") {
